...
function vsRangeFromOffsets(start, end, state){
  let startPos = state.buff.positionForCharacterIndex(start);
  let endPos = state.buff.positionForCharacterIndex(end);
...
function rangeOuterForm(state){
  let point = state.isPointImmediatelyFollowingForm || state.isPointImmediatelyFollowingExpression ?
    state.ogPointIdx-1
    :
    state.ogPointIdx;
  ...
  // if form only has one level of nesting
...
function endOfFormCondition(state, counter, depth) {
  ...
  let char = state.buffText.substring(counter, counter + 1);
  let formStartsArrayLog = ["offsets =>", "["].concat(formStartsArray, ["]"]);
...
function rangeCurrentForm (state, checkPreceding){
  ...
  let formStartsArray = state.allPos[point];
  console.log("rangeCurrentForm, checkPreceding is ", checkPreceding, " formStartsArray => ", formStartsArray);
  let depth = formStartsArray.length;
...
function blackListPos (idx, state, blackListedRangeIdx){
...
function addCharsForHashSet(text, formOffset, state){
  // This is for sets and things like: #{:a :b}
  if(/^{/.test(text)){
    if(util.charAtIdx(state, formOffset-1) === "#"){
    //if(charAtIndex(formOffset-1, state.buffText) === "#"){
      return "#"+text;
    }
  }
}
...
function addCharsForReaderConds(text, formOffset, state){
  // is this cljc a reader conditional ?
  if(/^\(/.test(text)){
...
function blackListAndIndexForms (state){
  ...
  // regex literal ranges or string ranges
  ...
  let idx = util.idxForPos(state, state.ogPoint);
...
function evalExpressionInsideForm(state){
  ...
  let point = state.isPointImmediatelyFollowingExpression ? state.ogPointIdx-1 : state.ogPointIdx;

  let formContents = state.textCurrentForm.substring(1).slice(0, -1);
  let ogIdx = state.ogPointIdx;
...
function evalExpressionOutsideForm(state){
  ...
  let char = state.buffText.substring(point, point+1);
  let idxEnd = point;
  let idxStart = point;
...
function isPointImmediatelyFollowingExpression(state){
...
function isPointImmediatelyFollowingForm(state){
  if(state.ogPointIdx === 0){
    return null;
  }
  let formStartsArray = state.allPos[state.ogPointIdx-1];
  console.log("isPointImmediatelyFollowingForm,  formStartsArray => ", formStartsArray);
  let depth = formStartsArray.length;
  ...
  let blackListedRangeIdx = state.blackListedPos[state.ogPointIdx-1];
  let blackListedRangeObj = state.blackListedRangesByIdx[blackListedRangeIdx];

  console.log("blackListedRangeIndex", blackListedRangeIdx);
...
function addFormRange(state){
  ...
  state.isPointImmediatelyFollowingForm = isPointImmediatelyFollowingForm(state);
  console.log("isPointFollowingForm??", state.isPointImmediatelyFollowingForm);
  state.isPointImmediatelyFollowingExpression = isPointImmediatelyFollowingExpression(state);
  console.log("isPointFollowingExpression??", state.isPointImmediatelyFollowingExpression);

  state.rangeCurrentForm = rangeCurrentForm(state, state.isPointImmediatelyFollowingForm);
  //state.rangeCurrentForm = rangeCurrentForm(state, false);

  //let immediatelyPrecedingForm = rangeCurrentForm ? rangeCurrentForm(state, true) : null;

  // add range info to state
  //state.rangeCurrentForm = currentForm || immediatelyPrecedingForm;
  //state.isPointImmediatelyFollowingForm = immediatelyPrecedingForm ? true : false;
